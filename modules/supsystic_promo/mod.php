...
class supsystic_promoOct extends moduleOct {
	...
	public function init() {
		...
		dispatcherOct::addFilter('showTplsList', array($this, 'checkProTpls'));
	}
	...
	public function getMainLink() {
		if(empty($this->_mainLink)) {
			$this->_mainLink = 'http://supsystic.com/plugins/octonis-plugin/';
		}
	...
	public function checkProTpls($list) {
		if(!$this->isPro()) {
			$imgsPath = frameOct::_()->getModule('octo')->getAssetsUrl(). 'img/tpl_prev/';
			$promoList = array(
			);
			foreach($promoList as $i => $t) {
				...
				$promoList[ $i ]['promo_link'] = $this->generateMainLink('utm_source=plugin&utm_medium='. $promoList[ $i ]['promo']. '&utm_campaign=octonis');
			}
			foreach($list as $i => $t) {
