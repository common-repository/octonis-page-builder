...
class octo_blocksModelOct extends modelOct {
	...
	protected function _afterGetFromTbl($row) {
		...
		if($row['cat_code'] == 'contacts') {
			$row['contact_form_start'] = frameOct::_()->getModule('contact')->generateFormStart( $row );
			$row['contact_form_end'] = frameOct::_()->getModule('contact')->generateFormEnd( $row );
	...
	public function save($d = array(), $oid = 0) {
		...
		if($id) {
			...
			if($originalId && ($originalBlock = $this->getById( $originalId ))) {
				...
				$originalBlock['sort_order'] = isset($d['sort_order']) ? $d['sort_order'] : 0;
