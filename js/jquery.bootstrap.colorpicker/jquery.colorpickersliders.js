        ...
        return this.each(function() {
            ...
            function init() {
                ...
                if (triggerelementisinput) {
					...
					/*OCTO*/
                    if (color.tiny/*!color.tiny.format*/) {
					/*****/
            ...
            function showPopup() {
				...
				/*OCTO*/
				var delta = 64;	// It was == 12 before
				/*****/
                if (offset.left + popuporiginalwidth + delta <= viewportwidth) {
                    container.css('left', offset.left).width(popuporiginalwidth);
                }
                else if (popuporiginalwidth <= viewportwidth) {
                    container.css('left', viewportwidth - popuporiginalwidth - delta).width(popuporiginalwidth);
                }
                else {
                    container.css('left', 0).width(viewportwidth - delta);
                }
            ...
            function _initSettings() {
                settings = $.extend({
					...
					onshow: false,
					onhide: false,
					noChangeAfterInit: false,	// By default - after first init it will trigger onchange event
            ...
            function _buildHtml() {
                ...
                if (settings.order.opacity !== false) {
                    sliders[settings.order.opacity] = '<div class="cp-slider-label">' + settings.labels.opacity + '</div><div class="cp-slider cp-opacity cp-transparency"><span></span><div class="cp-marker"></div></div><div style="clear: both;"></div>';
                }

                if (settings.order.hsl !== false) {
                    sliders[settings.order.hsl] = '<div class="cp-slider-label">' + settings.labels.hslhue + '</div><div class="cp-slider cp-hslhue cp-transparency"><span></span><div class="cp-marker"></div></div><div style="clear: both;"></div>'
							+ '<div class="cp-slider-label">' + settings.labels.hslsaturation + '</div><div class="cp-slider cp-hslsaturation cp-transparency"><span></span><div class="cp-marker"></div></div><div style="clear: both;"></div>'
							+ '<div class="cp-slider-label">' + settings.labels.hsllightness + '</div><div class="cp-slider cp-hsllightness cp-transparency"><span></span><div class="cp-marker"></div></div><div style="clear: both;"></div>';
                }
                ...
                if (settings.swatches) {
                    color_picker_html += '<div class="cp-swatches">'
							+ '<div class="hex_preset_shell"><div class="hex_present_pref">#</div><input type="text" class="hex_present" /></div>'
							+ '<button type="button" class="add" title="' + settings.titleswatchesadd + '"><i class="glyphicon glyphicon-plus"></i></button>'
							+ '<button type="button" class="remove" title="' + settings.titleswatchesremove + '"><i class="glyphicon glyphicon-minus"></i></button>'
							+ '<button type="button" class="reset" title="' + settings.titleswatchesreset + '"><i class="glyphicon glyphicon-refresh"></i></button>'
                ...
                if (settings.flat) {
					if(settings.appendto) {
						container = $('<div class="cp-container"></div>').appendTo(settings.appendto);
					} else if (triggerelementisinput) {
            ...
            function _bindEvents() {
				...
				/*OCTO*/
				container.find('.hex_present').keyup(function(){
					var colorValue = $(this).val()
					,	newColor = tinycolor( colorValue );
					if(newColor.isValid()) {
            ...
            function _resetSwatches() {
                if (confirm("Do you really want to reset the swatches?")) {
                    _parseCustomSwatches();
            ...
            function _updateAllElements(disableinputupdate) {
                ...
                if ((100 - color.cielch.l) * color.cielch.a < settings.previewcontrasttreshold) {
                    ...
                    if (triggerelementisinput && settings.previewontriggerelement) {
                        triggerelement.css('background-color', color.tiny.toRgbString()).css('color', '#000');
                    }
                }
                else {
                    elements.all_sliders.css('color', '#fff');
                    if (triggerelementisinput && settings.previewontriggerelement) {
                        triggerelement.css('background-color', color.tiny.toRgbString()).css('color', '#fff');
                    }
                }

                _findActualColorsSwatch();
				if(!settings.noChangeAfterInit || (settings.noChangeAfterInit && settings.notFirstUpdate)) {
					settings.onchange(container, color);
				}
				settings.notFirstUpdate = true;
            ...
            function _renderHue() {
                ...
                //setGradient(elements.sliders.hue, $.fn.ColorPickerSliders.getScaledGradientStops(freeColor, "h", 0, 360, 7));
				/*****/
				setGradient(elements.sliders.hue, $.fn.ColorPickerSliders.getScaledGradientStops(color.hsla, "h", 0, 360, 7));	// Old one
            ...
            function _renderLightness() {
				/*OCTO*/
				//var freeColor = $.extend({}, color.hsla);
				//freeColor.a = 1;
                //setGradient(elements.sliders.lightness, $.fn.ColorPickerSliders.getScaledGradientStops(freeColor, "l", 0, 1, 3));
				/*****/
                setGradient(elements.sliders.lightness, $.fn.ColorPickerSliders.getScaledGradientStops(color.hsla, "l", 0, 1, 3));	// Old one
