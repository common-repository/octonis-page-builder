...
class htmlOct {
    ...
    static public function img($src, $usePlugPath = 1, $params = array('width' => '', 'height' => '', 'attrs' => '')) {
        if($usePlugPath) $src = OCT_IMG_PATH. $src;
        return '<img src="'.$src.'" '
    ...
    static public function ajaxfile($name, $params = array('url' => '', 'value' => '', 'fid' => '', 'buttonName' => '')) {
		frameOct::_()->addScript('ajaxupload', OCT_JS_PATH. 'ajaxupload.js');
        $out = '';
        if(strpos($params['url'], 'pl='. OCT_CODE) === false)
			$params['url'] = uriOct::_(array('baseUrl' => $params['url'], 'pl' => OCT_CODE));
    ...
    static public function statesInput($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true, 'id' => '', 'selectHtml' => '')) {
        ...
        $states = fieldAdapterOct::getStates($params['notSelected']);
        ...
        if(empty($params['doNotAddJs'])) {
            $res .= '<script type="text/javascript">
                // <!--
                if(!toeStates.length)
                    toeStates = '. utilsOct::jsonEncode($states). ';
    ...
    static public function countryList($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true)) {
        $params['notSelected'] = isset($params['notSelected']) ? $params['notSelected'] : true;
        $params['options'] = fieldAdapterOct::getCountries($params['notSelected']);
    ...
    static public function textFieldsDynamicTable($name, $params = array('value' => '', 'attrs' => '', 'options' => array())) {
        ...
        if(!empty($params['options'])) {
            ...
            foreach($params['options'] as $key => $p) {
                $pattern[$key] = htmlOct::text($name. '[]['. $key. ']');
            }
            ...
            elseif(!is_array($params['value'])) {
                $params['value'] = utilsOct::jsonDecode($params['value']);
            ...
            do {
                ...
                foreach($params['options'] as $key => $p) {
                    ...
                    $res .= __($p['label']). htmlOct::text($name. '['. $i. ']['. $key. ']', $paramsForText);
                }
                $res .= $remove. '</div>';
                $i++;
    ...
    static protected function _loadProductsOptions() {
        if(empty(self::$productsOptions)) {
            $products = frameOct::_()->getModule('products')->getModel()->get(array('getFields' => 'post.ID, post.post_title'));
            if(!empty($products)) {
	...
	static public function fontsList($name, $params = array('value' => '')) {
		...
		if(empty($options)) {	// Fill them only one time per loading
			foreach(utilsOct::getFontsList() as $font)
				$options[ $font ] = $font;
		}
		$params['options'] = array();
		if(isset($params['default'])) {
			$params['options'] = array(OCT_DEFAULT => $params['default']);
		}
	...
	static public function slideInput($name, $params = array('attrs' => '', 'checked' => false, 'id' => '')) {
		...
		return '<a class="toeSlideShellOct" href="#"'. $params['attrs']. '>
			<span class="toeSlideButtOct"></span>
			<span class="toeSlideOnOct">'. __('ON'). '</span>
			<span class="toeSlideOffOct">'. __('OFF'). '</span>
	...
	static public function galleryBtn($name, $params = array()) {
		...
		$buttonParams = $params;
		$buttonParams['value'] = isset($params['btnVal']) ? $params['btnVal'] : sprintf(__('Select %s', OCT_LANG_CODE), strFirstUp($galleryType));
	...
	/*static public function radialProgress($progress, $attrs = array()) {
		frameOct::_()->addStyle('radial-progress', OCT_CSS_PATH. 'radial-progress.css');
