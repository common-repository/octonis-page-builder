...
abstract class tableOct {
    ...
    static public function getInstance($table = '') {
        ...
        if(!isset($instances[$table])) {
            $class = 'table'. strFirstUp($table). strFirstUp(OCT_CODE);
            ...
            if($id) {
                ...
                foreach($data as $field) {
                    ...
                    foreach($field as $k => $v) {
                        if(isset($this->_fields[$k])) {
                            $row[$k] = toeCreateObjOct('fieldOct', array(
                                    $this->_fields[$k]->name,
                                    $this->_fields[$k]->html,
                                    $this->_fields[$k]->type,
    ...
    public function getTable($transform = false) {
        if($transform)
            return dbOct::prepareQuery($this->_table);
        else
            return $this->_table;
    }
    ...
    public function get($fields = '*', $where = '', $tables = '', $return = 'all') {
        ...
        return dbOct::get($query, $return);
    }
    public function store($data, $method = 'INSERT', $where = '') {
		$this->_clearErrors();
        $method = strtoupper($method);
		if($this->_escape) {
			$data = dbOct::escape($data);
		}
        $query = '';
        switch($method) {
            case 'INSERT':
        ...
        $fields = $this->_getQueryString($data, ',', false);

        if(empty($fields)) {
            $this->_addError(__('Nothig to update', OCT_LANG_CODE));
            return false;
        }
        ...
        if(dbOct::query($query)) {
            if($method == 'INSERT')
                return dbOct::lastID();
            else
                return true;
        } else {
            $this->_addError(OCT_TEST_MODE ? dbOct::getError() : __('Database error. Please contact your developer.', OCT_LANG_CODE));
        }
        return false;
    }
            ...
            foreach($data as $k => $v) {
                if(array_key_exists($k, $this->_fields) || $k == $this->_id) {
                    ...
                    if(isset($this->_fields[$k]) && $this->_fields[$k]->adapt['dbTo']) 
                        $val = fieldAdapterOct::_($val, $this->_fields[$k]->adapt['dbTo'], fieldAdapterOct::DB);
                    if($validate) {
                        if (is_object($this->_fields[$k])) {
                            $objForValidation = clone $this->_fields[$k];
                            $objForValidation->setValue($val);
                            if($errors = validatorOct::_($objForValidation)) {
    ...
    /**
     * Add new fieldOctOct for children table (@see class field)
    ...
    protected function _addField($name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '') {
        $this->_fields[$name] = toeCreateObjOct('fieldOct', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
        return $this;
    }
	/**
    ...
    public function exists($value, $field = '') {
        if(!$field)
            $field = $this->_id;
        return dbOct::get('SELECT '. $this->_id. ' FROM '. $this->_table. ' WHERE '. $field. ' = "'. $value. '"', 'one');
    }
            ...
            switch($f->type) {
                case 'date':
                    if($d[$key] == '0000-00-00' || empty($d[$key]))
                        ...
                        $d[$key] = date(OCT_DATE_FORMAT, dbOct::dateToTime($d[$key]));
                    }
                    break;
                case 'int':
                case 'tinyint':
    ...
    public function getLastInsertID() {
        return dbOct::get('SELECT MAX('. $this->_id. ') FROM '. $this->_table, 'one');
    }
