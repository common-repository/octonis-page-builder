...
class validatorOct {
    ...
    static public function validate($field, $label = '', $validate = array()) {
        self::$errors = array();
        if(is_object($field) && get_class($field) != 'fieldOct') {
            $value = $field;
            $field = new fieldOctOct('noMatter');
            $field->label = $label;
        ...
        if(!empty($field->validate)) {
            foreach($field->validate as $v) {
                 if(method_exists('validatorOct', $v)) 
                    self::$v($field);
            }
        }
        if(method_exists('validatorOct', $field->type)) {
            $validate = $field->type;
            self::$validate($field);
        }
        if($field->maxlen) {
    ...
    static public function getProductValidationMethods() {
        $res = array();
        $all = get_class_methods('validatorOct');
        foreach($all as $m) {
