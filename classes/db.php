...
class dbOct {
     ...
     * #__  Woroctess prefix
     * ^__  Store plugin tables prefix (@see OCT_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, OCT_DB_PREF, $wpdb->prefix. OCT_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampOct($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, OCT_DATE_DL)) return dateToTimestampOct($date);
        $arr = explode('-', $date);
        return dateToTimestampOct($arr[2]. OCT_DATE_DL. $arr[1]. OCT_DATE_DL. $arr[0]);
    }
