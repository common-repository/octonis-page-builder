...
class errorsOct {
    ...
    static public function setSession($error) {
        ...
        if(is_array($error))
            ...
            $sesErrors[] = $error;
        reqOct::setVar('sesErrors', $sesErrors, 'session');
    }
    static public function init() {
        $octErrors = reqOct::getVar('octErrors');
        if(!empty($octErrors)) {
            if(!is_array($octErrors)) {
                $octErrors = array( $octErrors );
            }
            $octErrors = array_map('htmlspecialchars', array_map('stripslashes', array_map('trim', $octErrors)));
            if(!empty($octErrors)) {
                self::$current = $octErrors;
				if(is_admin()) {
					add_action('admin_notices', array('errorsOct', 'showAdminErrors'));
				} else {
					add_filter('the_content', array('errorsOct', 'appendErrorsContent'), 99999);
				}
            }
        }
    }
    ...
    static public function getSession() {
        return reqOct::getVar('sesErrors', 'session');
    }
    static public function clearSession() {
        reqOct::clearVar('sesErrors', 'session');
    }
