...
class utilsOct {
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsOct::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsOct::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameOct::_()->getModule($m)) 
                return false;
            if(!is_dir(frameOct::_()->getModule($m)->getModDir())) 
                return false;
        }
        return true;*/
        foreach(frameOct::_()->getModules() as $m) {
	...
	static public function isPluginsPage() {
		return (basename(reqOct::getVar('SCRIPT_NAME', 'server')) === 'plugins.php');
	}
	...
	/**
	 * Parse woroctess post/page/custom post type content for images and return it's IDs if there are images
