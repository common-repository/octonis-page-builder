...
class fieldAdapterOct {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterOct', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsOct'] = array();
    ...
    static public function userFieldDestToDB($value) {
        return utilsOct::jsonEncode($value);
    }
    static public function userFieldDestFromDB($value) {
        return utilsOct::jsonDecode($value);
    }
    static public function taxDataHtml($field) {
        ...
        $categories = frameOct::_()->getModule('products')->getCategories();
        $brands = frameOct::_()->getModule('products')->getBrands();
        $cOptions = array();
        $bOptions = array();
        if(!empty($categories)) {
        ...
        return '<div>'. __('Apply To', OCT_LANG_CODE). '
            <div id="tax_address">
                <b>'. __('Address', OCT_LANG_CODE). '</b><br />
                '. __('Destination', OCT_LANG_CODE). ':'. htmlOct::checkboxlist('params[dest]', array('optionsOct' => $listOfDest)). '<br />
                '. __('Country', OCT_LANG_CODE). ':'. htmlOct::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. __('Categories', OCT_LANG_CODE). '</b><br />
                '. (empty($cOptions) ? __('You have no categories', OCT_LANG_CODE) : htmlOct::checkboxlist('params[categories][]', array('optionsOct' => $cOptions))). '<br />
                    <b>'. __('Brands', OCT_LANG_CODE). '</b><br />
                '. (empty($bOptions) ? __('You have no brands', OCT_LANG_CODE) : htmlOct::checkboxlist('params[brands][]', array('optionsOct' => $bOptions))). '<br />
            </div>
            <div>'. __('Tax Rate', OCT_LANG_CODE).': '. htmlOct::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. __('Absolute', OCT_LANG_CODE).': '. htmlOct::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? __('Not selected', OCT_LANG_CODE) : __($notSelected);
			}
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frameOct::_()->getModule('optionsOct')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= __($key).':<br />';
                $output .= htmlOct::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= __('class', OCT_LANG_CODE).':<br />';
                $output .= htmlOct::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= __('id', OCT_LANG_CODE).':<br />';
                $output .= htmlOct::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsOct']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsOct'][0] = in_array(0,$field->value['categories'])?__('Deselect All'):__('Select All', OCT_LANG_CODE);
            foreach($categories as $c) {
                $field->htmlParams['optionsOct'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frameOct::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frameOct::_()->getTable('states')
				->leftJoin( frameOct::_()->getTable('countries'), 'country_id' )
